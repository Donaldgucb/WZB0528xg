    ...
    for (NSInteger i = 0; i < numberOfSections; i++) {
        ...
        if ([self.delegate respondsToSelector:@selector(collectionView:sizeForLargeItemsInSection:)]) {
            if (!CGSizeEqualToSize([self.delegate collectionView:self.collectionView sizeForLargeItemsInSection:i], RACollectionViewTripletLayoutStyleSquare)) {
                ...
                {
                    smallCellSize = CGSizeMake(largeCellSize.width,largeCellSize.height);
                }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:sizeForLargeItemsInSection:)]) {
        if (!CGSizeEqualToSize([self.delegate collectionView:self.collectionView sizeForLargeItemsInSection:indexPath.section], RACollectionViewTripletLayoutStyleSquare)) {
            ...
            {
                _smallCellSize = CGSizeMake(_largeCellSize.width,_largeCellSize.height);
            }
    ...
    if (CGSizeEqualToSize(_largeCellSize,_smallCellSize)) {
        ...
        {
            attribute.frame = CGRectMake(_insets.left+_largeCellSize.width+10, lineOriginY, _largeCellSize.width, _largeCellSize.height);
        }
        else if(indexPath.item==2)
        {
             attribute.frame = CGRectMake(_insets.left+_largeCellSize.width*2+20, lineOriginY, _largeCellSize.width, _largeCellSize.height);
        }
